...
export const fetchTwentyPharmacy = async ({ letter }: any) => {
    const res: any = await PHARMA.find({ 'contact.address.city': { $regex: '^' + letter, $options: 'i' } }).select('account contact');
    return res;
}

export const fetchTwentyByCity = async ({ city }: any) => {
    const res: any = await PHARMA.find({ 'contact.address.city': city.toUpperCase() }).select('account contact');
...
export const fetchPharmaDetails = async ({ name }: any) => {
    const res: any = await PHARMA.findOne({ 'account.name': name }).select('account contact');
    if (!res) return "no_result_founded";
    else return res;
}
export const searchPharmaByName = async ({ name }: any) => {
    const res: any = await PHARMA.find({ 'account.name': new RegExp(name, 'ig') }).select('account contact');
    if (!res) return "no_result_founded";
    else return res;
}
export const fetchPharmaById = async ({ id }: any) => {
    const res: any = await PHARMA.findById(id).select('account contact');
    if (!res) return "no_result_founded";
    else return res;
}